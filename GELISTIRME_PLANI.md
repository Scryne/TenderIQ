...
- [~] **Çoklu-organizasyon desteği:** login'de aktif üyelik seçimi + `POST /auth/switch-org` (bugün ilk üyelik rastgele seçiliyor); davet akışı (`Membership` + e-posta daveti). → **Sprint 3.3-E-1 (2026-07-22):** login artık **deterministik** (en erken üyelik; "rastgele org" bug'ı giderildi), `GET /auth/memberships` (org+rol listesi, aktif işareti) + `POST /auth/switch-org` (üye olunan org'a yeni token; üye değilse 403). **Kalan: davet akışı (3.3-E-2)** — e-posta seam'i (3.3-D) hazır; `Invitation` modeli + accept (mevcut/yeni kullanıcı çatalı) sonraki parçada.
