...
- [ ] **Ödeme entegrasyonu** (§14.3): iyzico/PayTR; **webhook imza doğrulama + idempotent işleme** (aynı webhook iki kez gelirse çift faturalama yok); abonelik↔kota senkronizasyonu; `UsageRecord` güncelleme (skill: `payment-integration-tr`). → **3.3-B** (`Subscription.provider*` alanları bu dilim için hazır).
