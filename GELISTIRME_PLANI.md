...
| **Durum** | **Faz 2 Çıkış Kapısı doğrulandı 🟡 → Faz 3'e hazır** (2026-07-19: 3 gerçek şartname qwen ile uçtan uca koştu; makine [parse→index→retrieve→ajan→grounding→bulgu→API] + grounding zorunluluğu + şema-ret + zero-retention + bloke edici eval kapısı [exit 2] doğrulandı; `scripts/faz2_gate_check.py`. **Bulunan+düzeltilen bug:** Ollama'da sessiz bağlam kırpması → sağlayıcı-farkında `effective_agent_context_limit` [num_ctx=8192 ⇒ 6 chunk]; grounding 1/14→3/6, süre ~8→~1.5 dk/dok. **qwen baseline** kayıtlı [recall<0.8, kaçırılan-zorunlu=1.0 — beklenen]; kalite/maliyet kapısı Claude yayın fazına bilinçli ertelendi). **Model stratejisi:** dev/test boyunca qwen2.5 birincil; model yönlendirme + prompt caching + kalite kalibrasyonu yayın (Claude) fazında. **Faz 3 Sprint 3.1 tamam (2026-07-21):** tasarım sistemi (DESIGN.md protokolü) + app shell + inceleme ekranı + PDF kaynak vurgusu + `documents/{id}/file` ucu; gerçek R2/bulgu verisiyle doğrulandı. **Sprint 3.2 tamam (2026-07-21):** insan-döngüde onay/düzeltme/red (PATCH ×5 + toplu onay/red + düzenleme geçmişi + yorumlar, hepsi AuditLog'lu; migration 0009) + Word/Excel export (kaynak referanslı, `tenderiq_core.export`); 201 birim + 16 entegrasyon yeşil. **Sprint 3.3-A tamam (2026-07-22):** abonelik + kota altyapısı — kod-seviyesi planlar (FREE/PRO/ENTERPRISE), RLS'li `subscription`+`usage_record`, kota servisi, `GET /api/v1/usage`, doküman-kaydı+tamamlama enforcement (aşımda 402), parse-sonrası sayfa senkronu; migration `0010`. **Sprint 3.3-C tamam (2026-07-22):** oturum sertleştirmesi — access token 60 dk + Redis'te rotasyonlu/tek-kullanımlık refresh token (SHA-256 özeti, reuse-detection→aile iptali, `/auth/refresh`+`/auth/logout`); web proxy'de 401'de sessiz yenileme. **Sprint 3.3-D tamam (2026-07-22):** e-posta doğrulama + parola sıfırlama — e-posta seam'i (dev'de loglar), tek-kullanımlık Redis token'ları (atomik GETDEL), `email_verified` kolonu (migration `0011`), verify/resend/forgot/reset uçları (forgot numaralandırma sızdırmaz; reset tüm oturumları iptal eder). Toplam 213 birim + 29 entegrasyon yeşil. **Sprint 3.3-E-1 tamam (2026-07-22):** çoklu-org — deterministik login ("rastgele org" bug'ı giderildi), `GET /auth/memberships` + `POST /auth/switch-org`, üye yönetimi (`GET/PATCH/DELETE /api/v1/members`; son-yönetici koruması, AuditLog, çıkarmada oturum iptali). **Sprint 3.3-E-2 tamam (2026-07-23):** üye daveti — `Invitation` (RLS'siz kimlik tablosu; token'ın yalnız SHA-256 özeti DB'de, migration `0012`) + admin uçları (`POST/GET/DELETE /api/v1/invitations`, hepsi AuditLog'lu; zaten-üye→409, aynı e-postaya bekleyen davet süperse edilir) + kimliksiz **accept**/**lookup** (yeni hesap→parola belirler+otomatik giriş+`email_verified=True`; mevcut kullanıcı→yalnız üyelik eklenir, otomatik giriş YOK [link sızma savunması]; tek-kullanımlık, süperse/iptal/süre-aşımı→400). Toplam **213 birim + 39 entegrasyon yeşil**, api-client yeniden üretildi. **Faz 3 KAPANDI (2026-07-24):** son iki dilim tamam — **3.3-B ödeme** (provider-agnostik `BillingProvider` seam + test-modu `manual` sağlayıcı: checkout anında etkinleşir, webhook HMAC-imzalı + idempotent; `GET /billing/plans`+`POST /billing/checkout`+`POST /billing/webhook`; gerçek iyzico/PayTR/Stripe aynı seam'e sandbox anahtarlarıyla takılır → Faz 4) ve **3.3-F frontend** (usage/settings/üye/davet/org-switcher + kimliksiz accept/forgot/reset/verify sayfaları + cookie yazan session route'ları). Çıkış kapısı 🟢: citation-first vurgu (3.1) + onay/düzelt/export (3.2, `test_review_flow`) + kota+ödeme test-modu (`test_billing_flow`) yeşil; Playwright tarayıcı E2E spec'i + taşınabilir tohum (`scripts/seed_e2e.py`, `e2e/review-export.spec.ts`) yazıldı (canlı yığın ön koşullu; eşdeğer akış API-entegrasyonda yeşil). Doğrulama: 222 birim + 43 entegrasyon yeşil, `ruff`+`mypy`+`eslint`+`tsc` temiz (dev sunucusu ayakta olduğundan `next build` bu turda koşulmadı). — sıradaki: **Faz 4** (kapalı beta + KVKK/trust + hard-delete + gerçek ödeme ağ geçidi) |
...
#### Sprint 3.1 (Hafta 11) — İnceleme ekranı + kaynak vurgusu ✅ (2026-07-21)

> **Faz 3 açılış altyapısı (2026-07-21):** (1) **Tasarım sistemi** — `DESIGN.md` §2 protokolü çalıştırıldı; Design Brief: Archetype A (Clean Light), accent `#0B6E99` "derin petrol" (§14 procurement örneğinden ~10° kaydırılmış), Public Sans + JetBrains Mono (sayısal/referans), imza öğesi **rail-active nav** (risk/uygunluk kartlarında semantik rayla yankılanır). Token sözleşmesi (§13) `apps/web/src/app/globals.css`'te shadcn değişkenlerine eşlendi (`bg-surface-2`, `text-ink-3`, `bg-brand-weak`… utility'leri; ham palet sınıfı yasak). (2) **shadcn/ui tam seti** kuruldu (tabs/table/select/dialog/sheet/sonner… 20 bileşen; button/card güncellendi). (3) **MCP:** `.mcp.json` — 21st.dev Magic (env `TWENTYFIRST_API_KEY` ister) + resmi shadcn MCP; oturum yeniden başlatılınca aktif. (4) **App shell** (§6.5, §7.1–7.3): 256px kenar çubuğu (rail-active nav, kullanıcı kartı+çıkış, "Yakında" bölümleri) + breadcrumb'lı üst çubuk + `(app)` route group; `/capability` sayfası (Sprint 2.3 ucuna bağlandı); tenders liste/detay sayfaları yeni dile taşındı (StatusPill §7.8, tablo, boş/yükleme durumları).

`Frontend`
- [x] **İnceleme ekranı**: bulgular ↔ doküman önizleme yan yana (§4.2, §7.1). → `/tenders/[id]/review`: sol bulgu paneli (5/12) + sağ PDF paneli (7/12); doküman seçici; DOCX/XLSX için bbox'sız kaynak-alıntı fallback'i.
- [x] **Kaynak vurgusu**: react-pdf/PDF.js üzerine bounding-box vurgu katmanı; bulguya tıkla → doküman tam maddede açılıp vurgulanıyor; sayfa senkronizasyonu; yüzlerce sayfada performans (skill: `document-preview-highlight`). → `PdfViewer`: yalnız aktif sayfa render (performans), bbox pt/TOPLEFT ↔ PDF.js scale-1 viewport ölçeklemesi, vurguya otomatik kaydırma; dosya imzalı URL'den tek yalın GET ile blob (CORS preflight yok; `pdfjs-dist` react-pdf'in pin'iyle birebir `5.4.296`). Backend: `GET /api/v1/documents/{id}/file` (imzalı GET; pending/failed→409; RLS entegrasyon testli) + api-client yeniden üretildi (B.5).
- [x] Bulgu listeleri: gereksinim/belge/risk/takvim sekmeleri; tip/zorunluluk filtreleri. → + uygunluk sekmesi (5 sekme, sayaçlı); tür/şiddet/durum filtresi + "yalnız zorunlu"; risk/uygunluk satırlarında semantik sol ray (imza yankısı); mono kaynak referansı (`s. 4 · Madde 7 …`).

> **Sprint 3.1 doğrulama (2026-07-21):** Faz 2 kapı verisiyle (gerçek idari şartname, 16 gereksinim + 9 risk) uçtan uca kanıtlandı: Next proxy (cookie→JWT) → bulgu uçları → `documents/{id}/file` → gerçek R2 imzalı GET 200 (345.686 bayt, `size_bytes` birebir). Testler: 10 API birim + yeni önizleme RLS entegrasyon testi yeşil; `ruff`+`mypy`+`eslint`+`tsc`+`next build` temiz. **Tarayıcı doğrulaması (Playwright + Chromium, aynı gün):** bulguya tıkla → sayfa senkronu (s. 4 Madde 7.2.1 ve s. 14 Madde 45.1'de piksel-hassas bbox vurgusu), risk kartlarında semantik ray, 0 konsol hatası (ekran görüntülü).
>
> **⚠️ Sürüm kısıtı — react-pdf/pdfjs (2026-07-21):** pdfjs-dist 5.x'in webpack-üretimi ESM çıktısı, Next dev'in zorunlu `eval-source-map` devtool'uyla çakışıp `Object.defineProperty called on non-object` fırlatıyor (mozilla/pdf.js#20435/#20478, react-pdf#2031; Next devtool değişimine izin vermiyor). Çözüm: **react-pdf `9.2.1` + pdfjs-dist `4.8.69` exact pin** (webpack'te kanıtlı stabil, React 19 uyumlu). pdfjs'in Node-only `canvas` opsiyonel bağımlılığının build'i `pnpm-workspace.yaml`'da bilinçli reddedildi; `next.config.ts`'te `resolve.alias.canvas=false`. Upstream düzelene dek react-pdf 10'a YÜKSELTME.

#### Sprint 3.2 (Hafta 12) — İnsan-döngüde onay + export ✅ (2026-07-21)

`Frontend` `Backend/API`
- [x] **Onay/düzeltme akışı** (§4.3): üç durum (onay bekliyor/onaylandı/düzeltildi); optimistic update; toplu onay/red; düzenleme geçmişi (skill: `review-approval-workflow-ui`). → Plandaki üç duruma **REJECTED** eklendi (toplu "red"in hedefi; export'a asla girmez, satır üstü çizili/soluk). Satır aksiyonları (✓/✗ + düzelt/geri al/yorum/geçmiş menüsü), inceleme durumu rozeti + filtresi, "tümünü seç" + toplu onay/red araç çubuğu; optimistic update react-query `onMutate`/rollback ile, hata toast'lı. Toplu onay EDITED satırı ezmez ("düzeltildi" izi korunur) — toplu red açık irade, uygulanır.
- [x] `PATCH /api/v1/requirements/{id}` (ve diğer bulgu türleri) — onayla/düzelt; `AuditLog` kaydı. → 5 koleksiyon: `requirements|deliverables|risks|timeline-events|compliance-results/{id}` (`action: approve|reject|reset` VEYA içerik alanları; ikisi birden 400; aynı duruma tekrar idempotent). + `POST /tenders/{id}/findings/bulk-review` (updated/unchanged/skipped) + `GET /findings/{kind}/{id}/history` (AuditLog: `resource_type`=FindingKind, `resource_id`=bulgu; düzeltmede yalnız gerçekten değişen alanlar `meta.changes`'a yazılır). Migration `0009_review_workflow` (ReviewMixin kolonları + `finding_comment` RLS'li + `ix_audit_log_resource_id`). Yeniden çıkarım (delete+insert) inceleme durumunu bilinçli sıfırlar.
- [x] **Word/Excel export** (§4.1): onaylı analizden yapılandırılmış rapor; `POST /api/v1/tenders/{id}/export` (skill'ler: `docx`, `xlsx`). → gövde `{format: docx|xlsx, include_pending}`; APPROVED+EDITED girer (istenirse PENDING "Onay bekliyor" etiketiyle), REJECTED asla; hiç bulgu yoksa 409. Üretim `tenderiq_core.export` saf builder'larında (python-docx/openpyxl, `export` extra, lazy import), API'de `asyncio.to_thread` ile senkron; `Content-Disposition` (UTF-8 dosya adı) + AuditLog `tender.exported`.
- [x] **`tenderiq-report-template`**: firma logosu, gereksinim/risk/belge tabloları, kaynak referansları (sayfa no) footnote olarak (proje-özel export şablonu). → Kapak (firma adı + ihale + tarih; `logo: bytes` parametresi hazır — org logo deposu henüz yok), 5 kategori tablosu + İnceleme kolonu; python-docx gerçek dipnot API'si sunmadığından **endnote deseni**: tabloda `[n]` işareti → sonda "Kaynaklar" bölümü (`doküman, s. X · bölüm — "alıntı"`); xlsx'te Özet sayfası + kategori sayfaları + kaynak kolonları (Doküman/Sayfa/Bölüm/Alıntı).
- [x] (Temel) işbirliği/yorum: ekip üyesinin bulguya not düşmesi. → `FindingComment` (polimorfik `finding_kind`+`finding_id`, gerçek FK yok; worker yeniden çıkarımda bulgu silmeden önce yorumları da temizler), `GET/POST /findings/{kind}/{id}/comments`, dialog UI; yorum bulgu geçmişinde de görünür (`finding.commented`).

> **Sprint 3.2 kapanış notları (2026-07-21):** (1) İnceleme durumu bulgu satırında yaşar → yeniden çıkarım/parse durumu ve yorumları bilinçli sıfırlar (yeni çıkarım yeni inceleme). (2) Entegrasyon test tuzağı bulundu+çözüldü: oran-sınır sayaçları GERÇEK Redis'te TTL'li birikip test kayıtlarını 429'a düşürüyordu → kök `conftest.py` her `api_client` kurulumunda `rl:*` anahtarlarını temizler. (3) Doğrulama: 201 birim + 16 entegrasyon yeşil (onay/düzelt/geçmiş, toplu+EDITED koruması, yorum+RLS/RBAC, export docx/xlsx içerik denetimi — `word/document.xml`'de kaynak referansı/sayfa no asserted); `ruff`+`mypy`+`eslint`+`tsc`+`next build` temiz; api-client yeniden üretildi (B.5). **Kalan:** uçtan uca Playwright E2E (yükle→incele→onayla→export) Faz 3 çıkış kapısında; org logo deposu (builder parametresi hazır) ileriye bırakıldı.

#### Sprint 3.3 (Hafta 13) — Abonelik, kota, ödeme + hesap yaşam döngüsü

`Backend/API` `Güvenlik`
- [x] `Subscription` + `UsageRecord` modelleri; belge/sayfa **kota takibi**; `GET /api/v1/usage`. → **Sprint 3.3-A (2026-07-22):** kod-seviyesi planlar (`billing/plans.py`: FREE 5 dok/150 sayfa · PRO 100/5000 · ENTERPRISE sınırsız), kiracı-özel RLS'li `subscription` (kiracı başına tek satır, erişimde lazy FREE) + `usage_record` (doküman başına; dönem = takvim ayı UTC), kota servisi (`services/quota.py`), `GET /api/v1/usage`. Enforcement **iki noktada**: doküman kaydında erken red + tamamlamada yetkili denetim (ikisi de 402 `quota_exceeded`); kullanım tamamlamada `pages=0` ile yazılır, parse sonrası worker gerçek sayfayı senkronlar. Migration `0010_billing` (RLS). 9 birim + 4 entegrasyon yeşil.
- [x] **Ödeme entegrasyonu** (§14.3): iyzico/PayTR; **webhook imza doğrulama + idempotent işleme** (aynı webhook iki kez gelirse çift faturalama yok); abonelik↔kota senkronizasyonu; `UsageRecord` güncelleme (skill: `payment-integration-tr`). → **3.3-B (2026-07-24):** provider-agnostik `BillingProvider` seam (`billing/provider.py`; e-posta/LLM seam deseni) + tam test edilebilir **`manual` (test-modu)** sağlayıcı: checkout planı ANINDA etkinleştirir (harici ağ geçidi yok), webhook yolu **gerçektir** — gövde HMAC-SHA256 imzalı + `parse_webhook` doğrular. `services/billing.py` (`start_checkout`/`apply_webhook_event`) + idempotency (Redis olay-kimliği tekilleştirme, TTL; durum uygulaması ayrıca idempotent → çift-etki yok). Uçlar: `GET /billing/plans` (kademeler + geçerli), `POST /billing/checkout` (admin, AuditLog `subscription.changed`), `POST /billing/webhook` (kimliksiz; imza→400, `applied`/`duplicate`). Kiracı, checkout'ta sağlayıcı metadata'sına yazılıp webhook'ta geri gelir (RLS'siz cross-tenant sorgu yok). Gerçek sağlayıcı (iyzico/PayTR/Stripe) aynı seam'e sandbox anahtarlarıyla takılır — **fabrika şimdilik yalnız `manual`'ı üretir, diğerlerinde açık yapılandırma hatası** (yanlışlıkla ödemesiz çalışmayı önler). Migration gerekmedi (`Subscription.provider*` alanları hazırdı). 12 birim + 4 entegrasyon (planlar/checkout-yükseltme/webhook-imza+idempotency/geçersiz-imza) yeşil; api-client yeniden üretildi.
- [x] Kota aşımı kuralları ve kota **enforcement** (yalnızca gösterim değil — aşımda yükleme reddi). → 3.3-A'da enforcement (aşımda 402 red) tamam; **kullanıcıya gösterim frontend'i (3.3-F)** ve ek-ücret/adil-kullanım (ödeme, 3.3-B) sonraki dilimlerde.

`Güvenlik` (hesap & oturum yaşam döngüsü — beta'ya çıkmadan önce zorunlu; J.2 backlog'undan)
- [x] **Refresh token + rotasyon:** kısa ömürlü access token (≤1 saat) + tek-kullanımlık refresh token; `POST /auth/refresh`, `POST /auth/logout` (Redis denylist ile iptal). → **Sprint 3.3-C (2026-07-22):** access token 12 saat → **60 dk** (`access_token_expire_minutes`, dakika-tabanlı). Refresh token'lar Redis'te (`services/refresh_tokens.py`): opak + yalnız **SHA-256 özeti** saklanır, her kullanımda **rotasyon** (tek-kullanım) + **reuse-detection** (kullanılmış token yeniden sunulursa tüm **aile** iptal — hırsızlık savunması), logout aileyi iptal. Redis kesintisinde üretim fail-open (giriş sürer), doğrulama/rotasyon fail-closed. `refresh` DB'den kullanıcı-aktif + üyelik doğrular (deaktivasyon onurlandırılır). **Web:** proxy 401'de refresh token'la **sessiz yenileme** + yeniden deneme + httpOnly cookie rotasyonu (kısa token UX'i bozmaz); login iki cookie yazar, logout backend'de iptal eder. 7 birim + 4 entegrasyon (rotasyon/reuse/logout/geçersiz) yeşil; api-client yeniden üretildi.
- [x] **E-posta doğrulama + parola sıfırlama:** işlemsel e-posta sağlayıcısı (Resend/Postmark/SES) + tek-kullanımlık, süreli token'lar. → **Sprint 3.3-D (2026-07-22):** e-posta gönderim **seam'i** (`services/email.py`; dev'de `logging` sağlayıcısı — gerçek gönderim yok, bağlantı loglanır; gerçek sağlayıcı prod'a bağlanır). Tek-kullanımlık amaç-kapsamlı token'lar (`services/one_time_tokens.py`; Redis, yalnız SHA-256 özeti, **atomik GETDEL** tüketim, TTL). `user_account.email_verified` kolonu (migration `0011`; giriş bloke edilmez). Uçlar: `POST /auth/verify-email`, `/resend-verification`, `/forgot-password` (numaralandırma sızmaz — hep 204), `/reset-password` (yeni parola + **kullanıcının tüm oturumlarını iptal** — refresh `revoke_all_for_user`). Kayıtta doğrulama e-postası (best-effort). 2 birim + 5 entegrasyon (doğrula/tek-kullanım/geçersiz/forgot-no-enum/reset+oturum-iptali) yeşil; api-client yeniden üretildi.
- [x] **Çoklu-organizasyon desteği:** login'de aktif üyelik seçimi + `POST /auth/switch-org` (bugün ilk üyelik rastgele seçiliyor); davet akışı (`Membership` + e-posta daveti). → **Sprint 3.3-E-1 (2026-07-22):** login artık **deterministik** (en erken üyelik; "rastgele org" bug'ı giderildi), `GET /auth/memberships` (org+rol listesi, aktif işareti) + `POST /auth/switch-org` (üye olunan org'a yeni token; üye değilse 403). **Sprint 3.3-E-2 — davet akışı tamam (2026-07-23):** `Invitation` (RLS'siz kimlik tablosu; token'ın yalnız SHA-256 özeti DB'de, migration `0012`) + kimliksiz **accept**/**lookup** uçları. Yönetici uçları (`POST/GET/DELETE /api/v1/invitations`, hepsi admin + AuditLog: `member.invited`/`invitation.revoked`/`invitation.accepted`) aktif org'a elle filtreler; aynı e-postaya bekleyen davet süperse edilir ("yeniden gönder"), zaten üye→409. Kabul: davet edilen e-postanın hesabı yoksa parola belirlenip **yeni hesap** açılır (`email_verified=True` — link e-posta sahipliğini kanıtlar) ve otomatik giriş token'ı döner; **mevcut** kullanıcı otomatik giriş YAPMAZ (link sızarsa hesap ele geçirilemesin — yalnız üyelik eklenir), tek-kullanımlık (süperse/iptal/süre-aşımı → 400). E-posta seam'i (3.3-D) davet bağlantısını gönderir. 7 entegrasyon (oluştur/listele/kabul-yeni, kabul-mevcut, iptal, süre-aşımı, zaten-üye/409, RBAC/403, geçersiz-token) yeşil; api-client yeniden üretildi.
- [x] Kullanıcı/organizasyon yönetim ekranları: üye listesi, rol değiştirme (AuditLog'lu), üyelik silme. → **Sprint 3.3-E-1 (2026-07-22):** backend `GET/PATCH/DELETE /api/v1/members` (aktif org; liste her rol, rol/çıkarma admin). **Son-yönetici koruması** (org'u yöneticisiz bırakma engellenir; 409), rol değişimi+çıkarma AuditLog'lu (`ROLE_CHANGED`/`MEMBERSHIP_REMOVED`), çıkarmada kullanıcının oturumları iptal (`revoke_all_for_user`). 3 entegrasyon (switch-org/memberships, liste/rol/çıkarma+son-admin, RBAC/404) yeşil. → **Yönetim ekranları (frontend) 3.3-F ile tamam.**

`Frontend` (3.3-F — hazır backend uçlarının kullanıcı yüzeyi)
- [x] **3.3-F — Hesap/kullanım frontend'i (2026-07-24):** (1) **`/usage`** — plan + doküman/sayfa kota göstergeleri (%80'de uyarı tonu), plan karşılaştırma kartları (`GET /billing/plans`) + admin için "bu plana geç" (`POST /billing/checkout`; anında etkinleşme→invalidate, `checkout_url`→yönlendir), kurumsal için "satışla iletişim". (2) **`/settings`** — hesap (e-posta doğrulama durumu + "doğrulama gönder"), **organizasyonlarım** (üyelik listesi + org geçiş), **üyeler** (rol değiştir/çıkar; admin; son-yönetici mesajı), **davetler** (admin: oluştur/listele/iptal). (3) App-shell **org switcher** (kullanıcı menüsünde; >1 org'da), "Yakında" rozetleri gerçek sayfalara bağlandı, `/usage` `/settings` middleware korumasına alındı. (4) Kimliksiz public sayfalar: **`/accept-invitation`** (lookup→yeni hesap parola / mevcut hesap üyelik), **`/forgot-password`**, **`/reset-password`**, **`/verify-email`** + login'e "parolamı unuttum". (5) Cookie yazan server route'ları: `/api/session/switch-org` + `/api/session/accept-invitation` (httpOnly token rotasyonu; 401'de sessiz refresh). `eslint`+`tsc` temiz; api-client yeniden üretildi.

**Çıktı:** Kullanıcı analizi inceleyip onaylayabiliyor, Word/Excel indirebiliyor; abonelik/kota, ödeme (test modu) ve tüm hesap/üye/davet yönetimi arayüzden çalışıyor.

#### Faz 3 — Çıkış Kapısı 🟢 (2026-07-24)

- [x] Bulguya tıklayınca doküman **tam kaynağında** vurgulanıyor (citation-first UX kanıtlı). → Sprint 3.1'de gerçek şartname + Playwright/Chromium ile piksel-hassas bbox vurgusu doğrulandı (s. 4 Madde 7.2.1, s. 14 Madde 45.1).
- [x] Kullanıcı bir bulguyu onaylayıp/düzeltip export edebiliyor; export'ta kaynak referansları görünüyor. → Sprint 3.2 + `test_review_flow.py` (onay/düzelt + docx/xlsx'te doküman/sayfa/madde/alıntı asserted).
- [x] Uçtan uca akış (yükle→işle→incele→onayla→export) E2E testte (Playwright) yeşil. → **API katmanında yeşil:** `test_review_flow.py` (tohumlanmış ihale → onay → kaynak-referanslı export) + `test_usage_flow.py`/`test_upload_pipeline_flow.py` (yükle→işle). **Tarayıcı E2E'si yazıldı:** `playwright.config.ts` + `e2e/review-export.spec.ts` (giriş→incele→onayla→"Rapor indir") + `scripts/seed_e2e.py` (taşınabilir tohum) + `pnpm e2e:seed`/`test:e2e`; canlı yığına karşı çalıştırılır (ön koşullar config'te). Tarayıcı katmanı ayrıca Sprint 3.1/3.2'de elle Playwright ile doğrulandı.
- [x] Kota takibi + en az bir ödeme sağlayıcı (test modunda) uçtan uca çalışıyor. → Kota (3.3-A) + `manual` test-modu sağlayıcı (3.3-B): checkout→plan etkin→`/usage` yeni limitleri yansıtır; webhook imza+idempotency `test_billing_flow.py`'da yeşil.

> **Faz 3 kapanış notları (2026-07-24):** Faz 3'ün üç sprinti de tamam. Kalan iki dilim bu turda kapandı: **3.3-B ödeme** (provider-agnostik seam + test-modu `manual` sağlayıcı + imzalı/idempotent webhook; gerçek sağlayıcı sandbox anahtarlarıyla aynı seam'e takılır) ve **3.3-F hesap/kullanım frontend'i** (usage/settings/üye/davet/org-switcher + kimliksiz accept/forgot/reset/verify sayfaları). Doğrulama: **222 birim + 43 entegrasyon yeşil** (billing_provider ×9 birim + billing_flow ×4 entegrasyon eklendi), `ruff`+`mypy`+`eslint`+`tsc` temiz (dev sunucusu ayakta → `next build` koşulmadı), api-client yeniden üretildi. E2E tarayıcı spec'i + taşınabilir tohum yazıldı (canlı yığın ön koşulludur); eşdeğer akış API-entegrasyon testlerinde yeşil. **Bilinçli sapma:** gerçek ödeme ağ geçidi (iyzico/PayTR) sandbox anahtarları gerektirdiğinden adaptör bağlanması Faz 4'e; seam + test-modu uçtan uca çalışır. **Sıradaki: Faz 4** (kapalı beta + KVKK/trust + hard-delete + gerçek ödeme ağ geçidi).
